...
void main() {
  ...
  group('P2 local store', () {
    ...
    test('titles stay raw; the untitled fallback is display-only', () async {
      ...
      expect(displaySessionTitle(AppStrings(AppLocale.zhHant), ''), '未命名對話');
      // A server title literally equal to the old fallback stays raw:
      expect(displaySessionTitle(AppStrings(AppLocale.en), '未命名對話'), '未命名對話');
    });
  ...
  group('STUCK-BUSY recovery budget (B3)', () {
    ...
    test(
      'beginPendingRecovery initializes once; reload keeps the deadline',
      () async {
        final st = await store();
        st.prefs.remove(kKey);
        ...
        expect(
          b1.record!.recoveryDeadline,
          kT0.add(const Duration(seconds: 60)),
        );
    ...
    test(
      'begin compare-mismatch never touches another token\u2019s record',
      () async {
        final st = await store();
        st.prefs.remove(kKey);
    ...
    test(
      'consumeRecoveryRetry is single-shot, re-arms exactly 30s once',
      () async {
        final st = await store();
        st.prefs.remove(kKey);
        await st.savePending('http://s', 'x', userText: 'q', runId: 'r');
        ...
        expect(
          c1.record!.recoveryDeadline,
          at.add(const Duration(seconds: 30)),
        );
    ...
    test(
      'clearPending compare-mismatch reports false and keeps the record',
      () async {
        final st = await store();
        st.prefs.remove(kKey);
        final token = (await st.claimPending(
