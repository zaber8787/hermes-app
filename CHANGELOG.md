...
## 2026-09-26 — 等待回覆的提示不再誤稱「連線中斷」

- 等待回覆時會顯示無輸出提示；長時間未收到串流更新時改以中性文字核對結果，僅在回覆串流結束或發生錯誤時提示中斷，不會自動重送訊息。

## 2026-09-22 — 對話列表批量隱藏與取消隱藏

- 對話列表新增多選隱藏與取消隱藏：可用「選取」或長按進入，並全選目前篩選後的列表。隱藏設定會同步至伺服器；部分失敗會保留選取供重試，已釘選的對話不會因此取消釘選。原有單筆操作移至每列的更多選單；舊版僅儲存在本機的隱藏設定會提示補同步。（伺服器 hidden 欄位契約屬既定目標，尚待跨裝置實測確認。）
